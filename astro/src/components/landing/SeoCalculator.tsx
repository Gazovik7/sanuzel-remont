...
import { ArrowRight, MessageCircle, Send, Phone, MessageSquare, TrendingUp, Calculator, HelpCircle } from 'lucide-react';
...
const SeoCalculator: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      if (typeof window !== 'undefined' && window.lead) {
          window.lead.capture(payload).then(ok => {
              if (ok) {
          ...
          console.log('Seo Calculator Lead:', payload);
          alert('Расчет отправлен! Ожидайте сообщения.');
          setFormData({ name: '', phone: '', url: '', budget: '' });
          setStep('calc');
      }
