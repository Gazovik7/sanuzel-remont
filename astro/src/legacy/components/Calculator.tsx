...
const Calculator: React.FC<CalculatorProps> = ({ onComplete, defaultFinish = 'Стандарт (Капитальный)' }) => {
  ...
  useEffect(() => {
    const maybeFocusArea = () => {
      if (typeof window === 'undefined') return;
      if (window.location.hash !== '#calculator-section') return;
      areaInputRef.current?.focus();
    };

    maybeFocusArea();
    window.addEventListener('hashchange', maybeFocusArea);
    return () => window.removeEventListener('hashchange', maybeFocusArea);
  }, []);
  ...
  return (
    <div id="calculator-section" className="bg-white rounded-[2rem] shadow-2xl overflow-hidden max-w-3xl mx-auto border border-gray-100 relative ring-1 ring-gray-100">
      {/* Top Bar */}
